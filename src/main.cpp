...
void setup() {
  ...
  pwm_input_mode_zero_timestamp_in_ms = millis();
#ifdef _ARDUINO_
  attachInterrupt(PWM_INPUT_IRQ_PIN, handle_pwm_input_interrupt, CHANGE);
#elif defined _ATTINY_ // register interrupt for attiny mcus
...
int current_input_mode() {
  ...
  int pwm_control_offset_in_us = pwm_in_us - PWM_CENTER;

  if (abs(pwm_control_offset_in_us) < PWM_CTL_HYSTERESIS) {
    return 0;
  }

  if (pwm_control_offset_in_us > 0) {
    return 1;
  }
...
void update_pwm_output(int pwm_width_in_us)
{
  DEBUG_PRINT("pwm: ");
...
void loop() {
  ...
  int input_mode = current_input_mode();

  DEBUG_PRINT("mode_in: ");
  DEBUG_PRINTLN(input_mode);

  if (input_mode == 0) {
    pwm_input_mode_zero_timestamp_in_ms = millis();
  }

  long input_mode_nonzero_timedelta_in_ms = millis() - pwm_input_mode_zero_timestamp_in_ms;

  if (input_mode_nonzero_timedelta_in_ms > MODE_CHANGE_TIMEDELTA_IN_MS) {

    int pwm_offset_in_us = input_mode * PWM_STEPSIZE;
    ...
    if (input_mode_nonzero_timedelta_in_ms > LONG_PRESS_TIME_IN_MS) {
      if (input_mode < 0) {
        pwm_output_pulse_width_in_us = PWM_LOWER_BOUND;
      } else {
        pwm_output_pulse_width_in_us = PWM_UPPER_BOUND;
      }
    }
  ...
  update_pwm_output(pwm_output_pulse_width_in_us);
